...
class InchToFtForm extends Component {
  constructor(props) {
    super(props)
  ...
  getOutput=(event)=>{
      event.preventDefault();
      this.setState({
        output:(this.state.input/12).toFixed(2),
        flag1:true
      })
  ...
  render(){
    ...
    <Grid container xs display="flex" minHeight={450} 
      ...
      <FormControl>
        <Typography variant="h5" color="primary" gutterBottom>INCH TO FT CONVERTER</Typography><br/>
        ...
        <FormControl sx={{textAlign:"center"}} >
          {this.state.flag1 && this.state.flag2 ?
            <Typography variant="subtitle1" color="primary" gutterBottom>
              {this.state.input} Inches is {this.state.output} Ft 
            </Typography>:<></>}
        </FormControl>
      </FormControl>
    </Grid>
